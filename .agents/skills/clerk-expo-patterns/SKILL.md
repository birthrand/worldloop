...
metadata:
  author: clerk
  version: 1.0.0
compatibility: Requires Expo CLI, expo-secure-store, expo-web-browser, and expo-auth-session
---
...
Install OAuth peer deps (included in the `expo-basic-auth` template):
...
| `expo-web-browser` module not found    | OAuth deps not installed                 | `npx expo install expo-web-browser expo-auth-session`                                     |
...
| Wrong password still completes sign-in | `signIn.password()` return value ignored | Destructure `{ error }` and return before reading `signIn.status` or calling MFA/finalize |
...
| `useSSO`                                 | `@clerk/expo`             |
| `WebBrowser.maybeCompleteAuthSession`    | `expo-web-browser`        |
