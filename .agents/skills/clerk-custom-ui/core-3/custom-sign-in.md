...
**Always check the returned `{ error }` before reading `signIn.status` or calling MFA/finalize.** Failed password attempts can leave a stale status from an earlier attempt; ignoring the return value can incorrectly complete sign-in.
...
export default function Page() {
  ...
  const handleMFAVerification = async (formData: FormData) => {
    ...
    // If you're using the authenticator app strategy, use verifyTOTP instead:
