...
type VisitedSpaceHeaderProps = {
  onReturn?: () => void;
  layout?: SavedCountriesLayout;
  onLayoutChange?: (layout: SavedCountriesLayout) => void;
};

type VisitedLayoutToggleProps = {
  layout: SavedCountriesLayout;
  onLayoutChange: (layout: SavedCountriesLayout) => void;
};

function VisitedLayoutToggle({
  layout,
  onLayoutChange,
}: VisitedLayoutToggleProps) {
  return (
...
export function VisitedSpaceHeader({
  onReturn,
  layout = "grid",
  onLayoutChange,
}: VisitedSpaceHeaderProps) {
  return (
    <View style={styles.row}>
      ...
      <Text className="font-semibold text-[18px] text-white">Visited</Text>
