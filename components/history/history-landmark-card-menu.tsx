...
type HistoryLandmarkCardMenuProps = {
  item: PlaceFeedItem;
...
export function HistoryLandmarkCardMenu({
  item,
  style,
}: HistoryLandmarkCardMenuProps) {
  const insets = useSafeAreaInsets();
  const [open, setOpen] = useState(false);
  const removeEntry = useRecentlyViewedStore((s) => s.removeEntry);
  ...
  const handleRemove = () => {
    closeMenu();
    removeEntry(historyEntryKey({ kind: "landmark", item, viewedAt: 0 }));
  };
    ...
    <>
      ...
      >
        ...
        <View style={styles.menuPanel}>
          <MenuRow
            label="Remove"
            subtitle="Remove from recently viewed"
            destructive
